...
/*
 * for(int i=0; i<list.size();i++) { Post post = list.get(i); String str =
 * "http://www.omdbapi.com/?t="+post.getTitle(); String str =
 * "https://api.themoviedb.org/3/search/movie" URL url = new URL(str);
 ...
 * (conn.getInputStream()))); filmy = br.toString(); }
 * 
 * BufferedReader br = new BufferedReader(new InputStreamReader(
 * (conn.getInputStream()))); filmy = br.toString();
 *
